module locked_token::upgrade_service_token;
...
public fun deposit<T>(upgrade_service_token: &mut UpgradeService<T>, upgrade_cap: UpgradeCap) {
    ...
    upgrade_service_token.assert_upgrade_cap_does_not_exist();
    let upgrade_cap_id = object::id(&upgrade_cap);
    upgrade_service_token.add_upgrade_cap(upgrade_cap);
...
public fun extract<T>(
    upgrade_service_token: &mut UpgradeService<T>,
    recipient: address,
    ctx: &TxContext,
) {
    upgrade_service_token.admin.assert_sender_is_active_role(ctx);
    upgrade_service_token.assert_upgrade_cap_exists();

    let upgrade_cap = remove_upgrade_cap(upgrade_service_token);
    let upgrade_cap_id = object::id(&upgrade_cap);
...
/// - Only callable by the admin.
public fun destroy_empty<T>(upgrade_service_token: UpgradeService<T>, ctx: &TxContext) {
    upgrade_service_token.admin.assert_sender_is_active_role(ctx);
    upgrade_service_token.assert_upgrade_cap_does_not_exist();

    let UpgradeService { id, admin } = upgrade_service_token;
    id.delete();
    admin.destroy();
...
public fun change_admin<T>(
    ...
    upgrade_service_token.admin.begin_role_transfer(new_admin, ctx)
}

/// Finalize admin role transfer process.
/// - Only callable by the pending admin.
public fun accept_admin<T>(upgrade_service_token: &mut UpgradeService<T>, ctx: &TxContext) {
    upgrade_service_token.admin.accept_role(ctx)
}
